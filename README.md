...
- **写用例**：页面表单化编辑，无需手写 YAML；支持断言可视化配置、变量依赖提示（接口/用例引用**裁剪**，见下方进度）
- **调试用例**：单步骤即时执行，不跑完整流程就能验证请求与响应，点击不超过 3 次
- **跑测试计划**：用例集 + 环境 + 触发方式（手动 / 定时 / CI）一体化配置（通知**裁剪**，见下方进度）
...
| 文档 | 内容 |
|:---|:---|
| [docs/用户验收指南.md](docs/用户验收指南.md) | **给使用者看的**：系统是什么、底层引擎与调用链路、从登录到建计划的一步步体验流程 |
...
| M2 · 计划与流水线 | 用例集 / 测试计划（定时）/ CI Token（接口定义库、通知**裁剪**） |
...
- ✅ **③ 编排层（用例集 / 测试计划 / CI Token）**：后端三片 + 前端三页已全部落地
  ...
  - ✅ 已完成：**编排层前端页面 + 浏览器端到端验收**
    ...
    - （headless 里 `navigator.clipboard` 默认被拒，脚本注入 stub 验证「复制成功后不再确认」的产品逻辑；
      剪贴板权限本身是环境，不是被测对象）
  - ⚠️ **M2 裁剪项（未实现，数据模型已预留）**：通知（企微/钉钉/Webhook，`notify_config` 表与
    `notify_config_id` 字段已建，发送器未做）、接口定义库 + 步骤引用（`api` / `testcase`
    步骤类型在编译时报「尚未开放」，`api_def` 表已建但无 CRUD）、WebSocket 进度推送
    （当前前端 1s 轮询 `GET /runs/{id}`）。
    详见 [项目方案 10.2](docs/项目方案.md) 与 [接口契约第 7 节](docs/接口契约.md)。
- ✅ **M3 调试与数据驱动（③ 三片全部落地）**
